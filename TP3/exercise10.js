...
// write a function that shws a student
function showStudent (student)
{
    // the lastname that we show is made of the first letter of the lastname with a uppercase letter 
    // and the substring of the lastname starting at the second letter with lowercase letters
    // we assume that each student has a lastname out of a single word
    var lastname = student.lastname[0].toUpperCase()+student.lastname.substring (1).toLowerCase();
    console.log (student.firstname.join (' ')+' '+lastname+' '+student.group);
}

function studentWithFirstname (db, fn)
{
    // filter the database (parameter db)
    // this returns a new array with all the elements
    // where the function returns true 
    return db.filter (function (student)
    {
        // return true if the firstname (parameter fn) is in the fisrtname list of the student
        if (student.firstname.indexOf (fn)>=0) return true;
        else return false;
    });
}

// sort the database using a comparison function
studentsDb.sort (function (student1, student2)
{
    // this function ois called with two parameters
    // that we need to compare
    // stduent1 and student2
    
    // first we compare the first letter of the lastname
    // if it is different, we return -1 or 1 
    // depending on the greater
    if (student1.lastname[0] < student2.lastname[0]) return -1;
    else if (student1.lastname[0] > student2.lastname[0]) return 1;
    // if they are equal
    else
    {
        // we compute the longest firstname for student 1
        var student1LongestFirstname = 0;
        student1.firstname.map (function (fn)
        {
            if (fn.length > student1LongestFirstname) student1LongestFirstname = fn.length; 
        });
        
        // we compute the longest firstname for student 2
        var student2LongestFirstname = 0;
        student2.firstname.map (function (fn)
        {
            if (fn.length > student2LongestFirstname) student2LongestFirstname = fn.length; 
        });
        
        // we compare the length of the longest firstname and return -1, 1 or 0 accordingly
        if (student1LongestFirstname < student2LongestFirstname) return -1;
        else if (student1LongestFirstname === student2LongestFirstname) return 0;
        else return 1;
    }
});
